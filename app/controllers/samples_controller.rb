...
#  end
  ...
  def list_sample_results
    # TODO
    # Probably put this in sample results controller
    # concatate Sample name with Barcode
    # write to sample_results. allow to choose multiple samples
    # get the list of linked samples from results_files from logged in's lab
    # compare lists from file sys to this list. If linked already, display different status on form greyed or whatever
    ...
    Dir.foreach('.') {
        ...
        @file_list.push([fn, File.size(fn), content_type]) if (fn[0].chr != '.')
    }
